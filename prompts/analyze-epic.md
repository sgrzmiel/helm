...
- **Anything framed as "single owner" / "bus factor" / "knowledge silo" risk.** It is normal at {company_name} for one developer to own a full FE or BE workstream on an initiative. Do NOT flag "single FE owner for the entire MVP build", "only one engineer assigned", "bus-factor risk", "knowledge concentrated on one person", etc. This is the standard staffing model, not a risk.
